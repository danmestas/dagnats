...
HTTP transport requires Bearer token authentication. `DAGNATS_BRIDGE_TOKEN` is the admin/root credential (unscoped, and the only credential the `/v1/tokens` routes accept); mint scoped, revocable worker tokens from it via `POST /v1/tokens` and hand those to individual machines instead. Workers must include `Authorization: Bearer {token}` header on all requests. Missing or invalid tokens return `401 Unauthorized`; a worker token polling outside its minted task-type prefixes returns `403`. See the REST API reference's Tokens section for the mint/list/revoke routes.
