	...
	for _, s := range stranded {
		...
		slog.Error(
			"possible stranded grouped task subject: #704 moved this "+
				...
				"legacy subject holds unacked messages this worker "+
				"will never receive. AMBIGUOUS: the same subject is "+
				"also the current, healthy one for the UNGROUPED task "+
				"type formed by joining the pair. Republish ONLY if "+
				"this deployment does not also run that ungrouped "+
				"task type -- republishing healthy work duplicates it",
