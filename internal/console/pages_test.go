...
// fakeDataSource composes every per-port fake from ds_fakes_test.go
// into the full DataSource surface. It exists so the ~70 page tests
// that just need "a console with data behind it" keep one constructor;
// a test that needs ONE port constructs that port's fake directly
// alongside deepUnimplemented instead.
//
// The two shared seeds (*fakeEntities, *fakeDeployment) are embedded
// here as well as inside the port fakes that read them. That is
// deliberate: Go resolves the shallowest selector, so `fake.runs` and
// `fake.configSnap` bind to the depth-1 seed unambiguously and every
// existing `fake.<field> = ...` call site keeps compiling.
...
// Compile-time proof the composition still covers the whole surface.
var _ DataSource = (*fakeDataSource)(nil)

// newFakeDS wires one entity seed and one deployment seed through every
// port fake that projects them, so a mutation made through one port is
// visible from the others exactly as it would be in production.
func newFakeDS() *fakeDataSource {
	...
	return &fakeDataSource{
		...
		fakeWorkerDirectory: fakeWorkerDirectory{
			fakeDeployment: deployment,
		},
		fakeSearchIndex: fakeSearchIndex{
			fakeEntities:    entities,
